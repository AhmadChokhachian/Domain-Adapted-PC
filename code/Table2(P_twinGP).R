...
OUTPUT_DETAIL <- file.path(RESULTS_DIR, "Table2_P_twinGP_detail.csv")
OUTPUT_SUMMARY <- file.path(RESULTS_DIR, "Table2_P_twinGP_summary.csv")
...
X2017_list <- vector("list", length(TURBINE_IDS))
y2017_list <- vector("list", length(TURBINE_IDS))
tid2017_list <- vector("list", length(TURBINE_IDS))
...
for (year in c(2017L, 2018L)) {
  ...
  for (target_id in test_ids) {
    cat("TwinGP - Turbine", target_id, "Year", year, "\n")
